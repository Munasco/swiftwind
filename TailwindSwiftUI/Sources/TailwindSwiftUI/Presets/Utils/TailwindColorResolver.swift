...
enum TailwindColorResolver {
    ...
    static func parseBracketColor(
        ...
        let innerRaw = String(valueStr.dropFirst().dropLast())
        guard let inner = unwrapOptionalDebugStringIfNeeded(innerRaw) else { return nil }
    ...
    private static func extractVariableToken(_ raw: String) -> String? {
        ...
        if trimmed.hasPrefix("var("), trimmed.hasSuffix(")") {
            ...
            if !TailwindVariableValidation.isColorThemeVariableToken(token) {
                TailwindLogger.warn(TailwindValidationMessages.nonNamespacedColorVariable(token))
            }
            return token
        }
        ...
        if trimmed.hasPrefix("("), trimmed.hasSuffix(")") {
            ...
            guard token.hasPrefix("--") else {
                TailwindLogger.warn(TailwindValidationMessages.varMustStartWithDoubleDash(token, example: "bg-(--brand-bg)"))
                return nil
            }
            if !TailwindVariableValidation.isColorThemeVariableToken(token) {
                TailwindLogger.warn(TailwindValidationMessages.nonNamespacedColorVariable(token))
        ...
        if trimmed.hasPrefix("--") {
            if !TailwindVariableValidation.isColorThemeVariableToken(trimmed) {
                TailwindLogger.warn(TailwindValidationMessages.nonNamespacedColorVariable(trimmed))
            }
            return trimmed
        }
    ...
    // Supports accidental Optional string interpolation:
    // Optional("#3B82F6"), Optional(var(--color-brand)), Optional(--color-brand)
    private static func unwrapOptionalDebugStringIfNeeded(_ raw: String) -> String? {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard trimmed.hasPrefix("Optional("), trimmed.hasSuffix(")") else { return trimmed }

        let start = trimmed.index(trimmed.startIndex, offsetBy: "Optional(".count)
        let end = trimmed.index(before: trimmed.endIndex)
        var payload = String(trimmed[start..<end]).trimmingCharacters(in: .whitespacesAndNewlines)

        if payload == "nil" || payload == "Optional(nil)" {
            TailwindLogger.warn("Optional color string resolves to nil: '\(raw)'")
            return nil
        }

        // String(describing: Optional("#fff")) includes quotes around payload.
        if payload.hasPrefix("\""), payload.hasSuffix("\""), payload.count >= 2 {
            payload = String(payload.dropFirst().dropLast())
        }

        if payload.isEmpty {
            TailwindLogger.warn("Optional color string is empty: '\(raw)'")
            return nil
        }
        return payload
    }
